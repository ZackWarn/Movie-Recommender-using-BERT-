...
def reduce_dataset(target_movies=10000, min_ratings=50):
    """
    Reduce dataset to top N movies by popularity and quality.
    ...
    Args:
        target_movies: Number of movies to keep (default: 10000)
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Movies: {len(movies):,} → {len(movies_filtered):,} ({len(movies_filtered)/len(movies)*100:.1f}%)")
    logger.info(f"Ratings: {len(ratings):,} → {len(ratings_filtered):,} ({len(ratings_filtered)/len(ratings)*100:.1f}%)")
    logger.info(f"Average ratings per movie: {len(ratings_filtered)/len(movies_filtered):.1f}")
    ...
    sample = movies_filtered.merge(top_movies[['score', 'rating_count', 'avg_rating']], 
                                   left_on='movieId', right_index=True)
    logger.info("\n" + sample.nlargest(10, 'score')[['title', 'rating_count', 'avg_rating']].to_string(index=False))
    
    return movies_filtered, ratings_filtered

if __name__ == "__main__":
    try:
        # Reduce to 10k movies
        reduce_dataset(target_movies=10000, min_ratings=50)
